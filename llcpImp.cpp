...
{
	//pointer to a pointer that points at headptr
	Node** cursor = &headPtr;
	
	if (headPtr == 0) //Empty list
	{
	   cerr << "DelOddCopEven() attempted on empty list" << endl;
	   return;
	}
	
	while (*cursor)
	{
	   //If a value is not divisible by 2 it is  
	   //odd node, remove it from the list.
	   if(((*cursor)->data % 2) != 0)
	   {
	      Node* current = *cursor;
		  *cursor = current->link;
	   ...
	   {
	   	  //If a value is divisible by 2 it is even node
	   	  //copy it to the list (make a duplicate).
